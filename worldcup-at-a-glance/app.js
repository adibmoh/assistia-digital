...
const state = { matches: [], standings: [], playerStats: [], sourceNotes: [], filter: "all", search: "", group: "all" };
...
async function fetchFirstAvailable(primaryUrl, fallbackUrl, label) {
  try {
    const data = await fetchJson(primaryUrl, `${label} API`);
    state.sourceNotes.push(`${label}: live API`);
    return data;
  } catch (apiError) {
    console.warn(apiError);
    try {
      const data = await fetchJson(fallbackUrl, `${label} fallback`);
...
function normalizeTeam(value) {
  if (value && typeof value === "object") return firstValue(value, ["name","team","country","title","short_name","full_name"], "TBD");
  return value ? String(value) : "TBD";
}

function normalizeVenue(value) {
  if (value && typeof value === "object") return firstValue(value, ["name","stadium","venue","city"], "");
...
function normalizeMatches(payload) {
  return unwrapPayload(payload).map(normalizeMatch).filter((m) => m.home && m.away);
}
...
function normalizeStandings(payload) {
  ...
  return raw.map((groupObj, idx) => {
    ...
    const teams = teamsRaw.map(normalizeStandingTeam).filter((t) => t.team && t.team !== "TBD")
...
function deriveStandingsFromMatches(matches) {
  ...
  for (const match of matches) {
    ...
    for (const team of [match.home, match.away]) {
      if (!table.has(team)) table.set(team, { team, played:0, wins:0, draws:0, losses:0, goalsFor:0, goalsAgainst:0, goalDiff:0, points:0 });
    }
    const h = table.get(match.home), a = table.get(match.away);
    ...
    else if (as > hs) { a.wins++; a.points += 3; h.losses++; }
    else { h.draws++; a.draws++; h.points++; a.points++; }
    h.goalDiff = h.goalsFor - h.goalsAgainst; a.goalDiff = a.goalsFor - a.goalsAgainst;
  }
  return [...groups.entries()].map(([group, table]) => ({
    group, teams: [...table.values()].sort((a,b) => b.points-a.points || b.goalDiff-a.goalDiff || b.goalsFor-a.goalsFor)
  }));
}
...
function extractPlayerStatsFromMatches(matches) {
  ...
  function add(name, field) {
    ...
    if (!players.has(key)) players.set(key, { name: String(name).trim(), goals: 0, assists: 0, mvp: 0 });
    players.get(key)[field] += 1;
  }
  for (const match of matches) {
    const row = match.original || {};
    const events = row.events || row.incidents || row.timeline || row.goals || [];
    if (Array.isArray(events)) {
      for (const event of events) {
        const type = String(firstValue(event, ["type","event","kind"], "")).toLowerCase();
        const player = firstValue(event, ["player","scorer","player_name","name"], "");
        const assist = firstValue(event, ["assist","assist_name","assisted_by"], "");
        if (/goal/.test(type) || event.scorer || event.player_name) add(player, "goals");
...
function statusLabel(status) { return status === "live" ? "Live" : status === "finished" ? "Finished" : "Upcoming"; }
...
function filteredMatches() {
  ...
  return state.matches.filter((m) => {
    ...
    if (q && !`${m.home} ${m.away} ${m.group} ${m.venue}`.toLowerCase().includes(q)) return false;
    return true;
  }).sort((a,b) => {
    const ad = a.date ? a.date.getTime() : Number.MAX_SAFE_INTEGER;
    const bd = b.date ? b.date.getTime() : Number.MAX_SAFE_INTEGER;
    return state.filter === "finished" ? bd-ad : ad-bd;
  });
}
...
function renderStandings() {
  ...
  root.innerHTML = state.standings.map((group) => `
    <section class="group-block"><h3>${escapeHtml(group.group)}</h3><div class="table-wrap"><table>
      <thead><tr><th>#</th><th>Team</th><th>Pts</th><th>P</th><th>W</th><th>D</th><th>L</th><th>GF</th><th>GA</th><th>GD</th></tr></thead>
      <tbody>${group.teams.map((t,i) => `<tr><td>${i+1}</td><td>${escapeHtml(t.team)}</td><td><strong>${t.points}</strong></td><td>${t.played}</td><td>${t.wins}</td><td>${t.draws}</td><td>${t.losses}</td><td>${t.goalsFor}</td><td>${t.goalsAgainst}</td><td>${t.goalDiff}</td></tr>`).join("")}</tbody>
...
function renderStats() {
  ...
  $("topScorers").innerHTML = scorers.length
    ? scorers.map((p,i) => `<div class="stat-item"><span class="rank">${i+1}</span><span>${escapeHtml(p.name)}</span><strong>${p.goals} goal${p.goals===1?"":"s"}${p.assists ? `, ${p.assists} assist${p.assists===1?"":"s"}` : ""}</strong></div>`).join("")
    : `<div class="empty">Top scorers are not available from the live source. No manual scorer data is shown.</div>`;

  const extras = state.playerStats.filter((p) => p.assists > 0 || p.mvp > 0).slice(0, 8);
  $("extraStats").innerHTML = extras.length
    ? extras.map((p,i) => `<div class="stat-item"><span class="rank">${i+1}</span><span>${escapeHtml(p.name)}</span><strong>${p.assists ? `${p.assists} assist${p.assists===1?"":"s"}` : ""}${p.assists && p.mvp ? " · " : ""}${p.mvp ? `${p.mvp} MVP` : ""}</strong></div>`).join("")
    : `<div class="empty">Assists and MVP data are not available from the live source. No manual MVP/assist data is shown.</div>`;
}

function renderAll() { renderGroupOptions(); renderMatches(); renderStandings(); renderStats(); }
...
async function loadData() {
  ...
  try {
    const [gamesPayload, groupsPayload] = await Promise.all([
      fetchFirstAvailable(ENDPOINTS.apiGames, ENDPOINTS.fallbackMatches, "matches"),
      fetchFirstAvailable(ENDPOINTS.apiGroups, ENDPOINTS.fallbackGroups, "standings")
    ]);
    ...
    $("statusText").textContent = `Loaded · ${state.sourceNotes.join(" · ")}`;
    $("lastUpdated").textContent = new Intl.DateTimeFormat(undefined, { dateStyle:"medium", timeStyle:"short" }).format(new Date());
    renderAll();
  } catch (error) {
    console.error(error);
...
function setupEvents() {
  ...
  $("searchInput").addEventListener("input", (event) => { state.search = event.target.value; renderMatches(); });
  $("groupFilter").addEventListener("change", (event) => { state.group = event.target.value; renderMatches(); });
